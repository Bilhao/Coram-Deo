...
class Biblia {
  ...
  Future<List<String>> getBooks(String testament) async {
    ...
    final List<Map<String, dynamic>> maps = await db.rawQuery(
        'SELECT DISTINCT book FROM bible WHERE testament = ?', [testament]);
  ...
  Future<List<int>> getChapters(String book) async {
    ...
    final List<Map<String, dynamic>> maps = await db
        .rawQuery('SELECT DISTINCT chapter FROM bible WHERE book = ?', [book]);
  ...
  Future<List<String>> getVersesId(String book, int chapter) async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps = await db.rawQuery(
        'SELECT verse_id FROM bible WHERE book = ? AND chapter = ?',
        [book, chapter]);
  ...
  Future<String> getBookById(int bookId) async {
    final db = await initDb();
    final maps = await db.rawQuery(
        'SELECT DISTINCT book FROM bible WHERE book_id = ?', [bookId]);
  ...
  Future<int> getLastChapterOfBook(String book) async {
    final db = await initDb();
    final maps = await db
        .rawQuery('SELECT MAX(chapter) FROM bible WHERE book = ?', [book]);
  ...
  Future<int> getBookId(String book) async {
    final db = await initDb();
    final maps =
        await db.rawQuery('SELECT book_id FROM bible WHERE book = ?', [book]);
