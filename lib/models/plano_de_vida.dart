...
class PlanoDeVida {
  ...
  Future<int> getId(String title) async {
    ...
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT id FROM data WHERE title = ?', [title]);
    return maps[0]['id'];
  }
  ...
  Future<bool> getIsCustom(String title) async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT isCustom FROM data WHERE title = ?', [title]);
  ...
  Future<bool> getIsSelected(String title) async {
    ...
    final List<Map<String, dynamic>> maps = await db
        .rawQuery('SELECT isSelected FROM data WHERE title = ?', [title]);
  ...
  Future<bool> getIsCompleted(String title) async {
    ...
    final List<Map<String, dynamic>> maps = await db
        .rawQuery('SELECT completedDates FROM data WHERE title = ?', [title]);
  ...
  Future<bool> getIsNotification(String title) async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps = await db
        .rawQuery('SELECT isNotification FROM data WHERE title = ?', [title]);
  ...
  Future<Map<String, dynamic>> getTitleAndNotificationTimes() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title, notificationTimes FROM data');
  ...
  Future<Map<String, dynamic>> getTitleAndCompletedDates() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title, completedDates FROM data');
    Map<String, dynamic> resultMap = {};
    for (Map<String, dynamic> map in maps) {
  ...
  Future<List<String>> getAllTitles() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title FROM data ORDER BY id');
  ...
  Future<List<String>> getTitleisCustom() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title FROM data WHERE isCustom = 1');
    return List.generate(maps.length, (i) {
      return maps[i]['title'];
    });
  }
  ...
  Future<List<String>> getTitleisSelected() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title FROM data WHERE isSelected = 1');
    return List.generate(maps.length, (i) {
      return maps[i]['title'];
    });
  }
  ...
  Future<List<String>> getTitleisCompleted(String date) async {
    ...
    final List<Map<String, dynamic>> maps = await db.rawQuery(
        'SELECT title FROM data WHERE completedDates LIKE ?', ["%$date%"]);
    return List.generate(maps.length, (i) {
      return maps[i]['title'];
    });
  }
  ...
  Future<List<String>> getTitleisNotification() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT title FROM data WHERE isNotification = 1');
    return List.generate(maps.length, (i) {
      return maps[i]['title'];
    });
  }
  ...
  toggleisSelected(String title) async {
    ...
    if (maps[0]['isSelected'] == 1) {
      await db.rawUpdate(
          'UPDATE data SET isSelected = ? WHERE title = ?', [0, title]);
    } else {
      await db.rawUpdate(
          'UPDATE data SET isSelected = ? WHERE title = ?', [1, title]);
    }
  }
  ...
  activateisNotification(String title) async {
    ...
    if (maps[0]['isNotification'] == 0) {
      await db.rawUpdate(
          'UPDATE data SET isNotification = ? WHERE title = ?', [1, title]);
    }
  }
  ...
  deactivateisNotification(String title) async {
    ...
    if (maps[0]['isNotification'] == 1) {
      await db.rawUpdate(
          'UPDATE data SET isNotification = ? WHERE title = ?', [0, title]);
    }
  }
  ...
  insertNotificationTime(String title, String time) async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps = await db.rawQuery(
        'SELECT notificationTimes FROM data WHERE title = ?', [title]);
    if (maps[0]['notificationTimes'] == null) {
      await db.rawUpdate(
          'UPDATE data SET notificationTimes = ? WHERE title = ?',
          [time, title]);
    } else {
      await db.rawUpdate(
          'UPDATE data SET notificationTimes = ? WHERE title = ?',
          [maps[0]['notificationTimes'] + ",$time", title]);
    }
  }
  ...
  insertCompletedDate(String title, String date) async {
    ...
    if (maps[0]['completedDates'] == null) {
      await db.rawUpdate(
          'UPDATE data SET completedDates = ? WHERE title = ?', [date, title]);
    } else {
      await db.rawUpdate('UPDATE data SET completedDates = ? WHERE title = ?',
          [maps[0]['completedDates'] + ",$date", title]);
    }
  }
  ...
  deleteCompletedDate(String title, String date) async {
    ...
    if (maps[0]['completedDates'].split(",").length == 1) {
      await db.rawUpdate(
          'UPDATE data SET completedDates = ? WHERE title = ?', [null, title]);
    } else {
  ...
  insertNewItem(String title, int isCustom, int isSelected, int isCompleted,
      int isNotification) async {
    final db = await initDb();
    await db.rawInsert(
        "INSERT INTO data(title, isCustom, isSelected, isCompleted, isNotification) VALUES(?, ?, ?, ?, ?)",
  ...
  getTitleAndNotificationId() async {
    final db = await initDb();
    final List<Map<String, dynamic>> maps =
        await db.rawQuery('SELECT id, title, notificationTimes FROM data');
