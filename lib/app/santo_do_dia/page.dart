...
class _SantoDoDiaPageState extends State<SantoDoDiaPage> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      ...
      child: Consumer2<SantoDoDiaProvider, AppProvider>(
        builder: (context, provider, fs, child) {
            ...
            if (provider.error != null) {
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text("Não foi possível conectar ao servidor para carregar o Santo do Dia. Verifique sua conexão com a internet."),
                  actions: <Widget>[
                    TextButton(
                      child: const Text('OK'),
                      onPressed: () {
          ...
          return Scaffold(
            appBar: AppBar(
              title: const Text("Santo do Dia", maxLines: 2, style: TextStyle(fontSize: 20)),
              actions: [
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    IconButton(
                                      onPressed: () async {
                                        DateTime? pickedDate = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2030), initialDate: DateTime(DateTime.now().year, provider.month, provider.day));
                                        if (pickedDate != null) {
                            ...
                            for (String text in provider.text)
                              Align(
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(fontSize: provider.boldText.contains(text) ? fs.fontSize + 5 : fs.fontSize + 2, fontWeight: provider.boldText.contains(text) ? FontWeight.bold : FontWeight.normal),
                                ),
                              ),
